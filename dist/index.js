...
function createGithubClient() {
    ...
    github.authenticate({
        type: "token",
        token: "640d9f33e7daef80be281b8ad9fb4c5c9457d02a"
    });
...
function evalNeededAction(reviewer, pr, repoData) {
    ...
    lastCommentFromAnyone = _maxBy(_.filter(pr.comments, function(comment){ return comment.body.indexOf("+1") === -1;}), function(comment){ return comment.updated_at});
    
    if(data.debug) console.log("%s should take a look at %s in %s", reviewer.name, pr.number, repoData.name);
    
    if(pr.user.login !== reviewer.username) {
        if(userComments.length) {
            if(lastComment.updated_at > pr.head.repo.pushed_at) {
                if(data.debug) console.log("%s has commented on %s", reviewer.username, lastComment.updated_at);
            } else {
                if(data.debug) console.log("%s has commented on %s but the comment is old (%s)", reviewer.username, lastComment.updated_at, pr.head.repo.pushed_at);
                reviewer.prs.push(pr);
            }  
        } else {
            if(data.debug) console.log("%s needs to look at this PR", reviewer.username);
            reviewer.prs.push(pr);
        } 
    } else if(!pr.mergeable) {
        if(data.debug) console.log("%s we're conflicted here");
        reviewer.prs.push(pr);
    } else {
        if(lastCommentFromAnyone && lastCommentFromAnyone.updated_at > pr.head.repo.pushed_at) {
            if(data.debug) console.log("Someone has made a comment in your PR");
            reviewer.prs.push(pr);
        } else {
            if(data.debug) console.log("You're the author and we're ok");
        }
